            ...
            {bookings.map((booking, index)=>(
              <tr key={index} className='border-t border-borderColor text-gray-500'>
                ...
                <td className='p-3 flex items-center gap-3'>
                  <img src={(booking.item || booking.car)?.image || ''} alt="" className='h-12 w-12 aspect-square rounded-md object-cover'/>
                  <p className='font-medium max-md:hidden'>{(booking.item || booking.car)?.title || 'Unknown'} {(booking.item || booking.car)?.model || ''}</p>
                </td>
